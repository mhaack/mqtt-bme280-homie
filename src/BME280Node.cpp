...
HomieSetting<long>
    sensorIntervalSetting("sensorInterval",
                          "The sensor reading interval in seconds");
HomieSetting<double>
...
void BME280Node::loop() {
  if (millis() - lastMeasurement >= sensorIntervalSetting.get() * 1000UL ||
      lastMeasurement == 0) {
    ...
    Homie.getLogger() << "Temperature (after offset): " << temperature << " °C"
                      << endl;
